...
#Random Forest
rfc_blanks = RFC(max_depth=6).fit(X_train, y_train.to_numpy().ravel())
rfc_y_test_predictions_blanks = rfc_blanks.predict(test_blank_counts)
...
#Random Forest
rfc_nonblanks = RFC(max_depth=6).fit(X_train, y_train.to_numpy().ravel())
rfc_y_test_predictions_nonblanks = rfc_nonblanks.predict(test_nonblank_counts)
...
#Neural Network
nn_blanks = MLPClassifier(hidden_layer_sizes=100).fit(X_train, y_train.to_numpy().ravel())
nn_y_test_predictions_blanks = nn_blanks.predict(test_blank_counts)
...
#Neural Network
nn_noblanks = MLPClassifier(hidden_layer_sizes=100).fit(X_train, y_train.to_numpy().ravel())
nn_y_test_predictions_noblanks = nn_noblanks.predict(test_nonblank_counts)
